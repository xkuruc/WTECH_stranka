...
<main>
    <section class="product_main_info">
        <form id="upload_images" method="POST" class="product_main_info_container" enctype="multipart/form-data" data-action="edit" data-product-id="{{ $produkt->id }}">
            <div class="product_images">
                <div class="product_images_kontajner">

                    @php
                        /* hlavný obrázok */
                        $mainImage = $produkt->images->firstWhere('is_main', true);

                        /* vedľajšie obrázky */
                        $sideImages = $produkt->images->where('is_main', false)->sortBy(function($image) { /* zoradíme podľa čísla v názve */
                        ...
                        $add_index = $offset + count($sideImages ?? []) + 1;
                    @endphp

                    <div class="big_images" id="sliding_container">

                        @if(isset($mainImage) && $mainImage->image_path)
                            <div class="big_img selected" data-img-id=1>
                                <input type="file" class="file-input" accept="image/*" multiple>
                                <img draggable="false" src="{{ asset('images/'.$mainImage->image_path) }}" alt="{{ $produkt->name }}">
                                <span style="display: none" class="plusko"></span>
                        ...
                        @foreach($sideImages as $image)
                            <div class="big_img" data-img-id="{{ $loop->iteration + $offset }}">
                                <input type="file" class="file-input" accept="image/*" multiple>
                                <img src="{{ asset('images/'.$image->image_path) }}" alt="{{ $produkt->name }}">
                                <span style="display: none" class="plusko"></span>
                                <button class="trash_can_right">
                                    <img src="{{ asset('./icons/kos_icon.png') }}" alt="">
                                </button>
                        ...
                        <div class="big_img add {{ is_null($mainImage) ? 'selected' : '' }}" data-img-id="{{ $add_index }}">
                            <input type="file" class="file-input" accept="image/*" multiple>
                            <img src="" alt="">
                    ...
                    <div class="small_images" id="small_slider">
                        ...
                        @foreach($sideImages as $image)
                            <div class="small_img" data-img-id="{{ $loop->iteration + $offset }}">
                                <input type="file" class="file-input" accept="image/*" multiple>
                                <img src="{{ asset('images/'.$image->image_path) }}" alt="{{ $produkt->name }}">
                                <span style="display: none" class="plusko"></span>
                            </div>
                        ...
                        <div class="small_img add {{ is_null($mainImage) ? 'selected' : '' }}" data-img-id="{{ $add_index }}">
                            <input type="file" class="file-input" accept="image/*" multiple>
                            <img src="" alt="">
                            <span class="plusko"></span>
                        </div>
            ...
            <div class="produkt_information">
                <div class="custom_input">
                    ...
                    <input class="input_edit" name="name" required value="{{ $produkt->name }}">
                </div>

                <div class="custom_input">
                    <span class="label_edit">Stručný popis:</span>
                    <textarea class="input_edit_popis" name="description" required>{{ $produkt->description ?? '' }}</textarea>
                </div>
                ...
                <div class="select_label">
                    ...
                    <select class="select_edit" name="brand_id" required> <!-- brand_id sa posúvajú do form -->
                        ...
                        @foreach ($brands as $brand)
                            <option value="{{ $brand->id }}" {{ $brand->id == $produkt->brand_id ? 'selected' : '' }}>
                                {{ $brand->display_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="select_label">
                    ...
                    <select class="select_edit" name="gender" required>
                        <option value="Pánske" {{ $produkt->gender === 'Pánske' ? 'selected' : '' }}>Pánske</option>
                        <option value="Dámske" {{ $produkt->gender === 'Dámske' ? 'selected' : '' }}>Dámske</option>
                        <option value="Unisex" {{ $produkt->gender === 'Unisex' ? 'selected' : '' }}>Unisex</option>
                    </select>
                </div>
                ...
                <div class="select_label">
                    ...
                    <select class="select_edit" name="color_id" required>
                        @foreach ($colors as $color)
                            <option value="{{ $color->id }}" {{ $color->id == $produkt->color_id ? 'selected' : '' }}>
                                {{ $color->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="select_label">
                    ...
                    <select class="select_edit" name="type" required>
                        <option value="Tenisky" {{ $produkt->type === 'Tenisky' ? 'selected' : '' }}>Tenisky</option>
                        <option value="Kopačky" {{ $produkt->type === 'Kopačky' ? 'selected' : '' }}>Kopačky</option>
                        <option value="Lopty" {{ $produkt->type === 'Lopty' ? 'selected' : '' }}>Lopty</option>
                    </select>
                </div>
                ...
                <div class="joined_input">
                    <div class="custom_input" >
                        ...
                        <input class="input_edit" type="number" min="0" name="price" required value="{{ $produkt->price }}">
                    </div>

                    <div class="custom_input">
                        <span class="label_edit">Zľava:</span>
                        <input class="input_edit" type="number" min="0" max="100" name="discount" required value="{{ $produkt->discount }}">
                    </div>
                </div>
                ...
                <div class="select_label">
                    ...
                    <div class="select_label dropdown" onclick="toggleDropdown('sizesSelect')">
                        <input type="text" id="selectedSizes" class="select_edit" placeholder="Preferovaná veľkosť oblečenia" readonly="" value="{{ old('sizes', $sizes_input) }}">
                        <input type="hidden" name="sizes" id="sizesHidden" value="{{ old('sizes', $sizes_input) }}">
                ...
                <button type="submit" class="button_pridat">Uložiť zmenu</button>
            </div>
        </form>
    </section>
</main>
